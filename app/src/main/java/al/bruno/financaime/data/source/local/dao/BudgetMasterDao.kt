...
interface BudgetMasterDao {
    ...
    @Query("SELECT b._id AS _id, b._budget -  TOTAL(e._amount) AS _budget, b._incomes AS _incomes, b._date AS _date, TOTAL(e._amount) AS _amount FROM budget AS b JOIN expense AS e ON b._id = e._id_budget WHERE strftime('%m', datetime(b._date/1000, 'unixepoch')) = :month GROUP BY e._id_budget")
